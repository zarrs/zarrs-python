...
pub trait ChunkConcurrentLimitAndCodecOptions {
    ...
    async fn get_chunk_concurrent_limit_and_codec_options_async(
        &self,
        codec_pipeline_impl: &AsyncCodecPipelineImpl,
    ) -> PyResult<Option<(usize, CodecOptions)>>;
}
    ...
    T: ChunksItem,
{
    ...
    fn get_chunk_concurrent_limit_and_codec_options_async(
        &self,
        codec_pipeline_impl: &AsyncCodecPipelineImpl,
    ) -> PyResult<Option<(usize, CodecOptions)>> {
        let num_chunks = self.len();
